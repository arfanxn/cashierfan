<template>
    ...
    <GuestLayout>
        <Card class="w-full lg:w-4/12">
            <form
                @submit.prevent="form.patch(route(`users.forgot-password`))"
                class="py-2 space-y-4"
            >
                <div class="text-slate-700">
                    <h1 class="text-xl font-normal uppercase">
                        Forgot Password
                    </h1>
                </div>
                ...
                <div class="flex items-center bg-gray-200 rounded">
                    ...
                    />
                    <Button @click="sendCode()" class="px-2 py-1 rounded-r"
                        >SEND</Button
                    >
                </div>
                <Alert
                    @onClose="form.errors.verification_code = null"
                    :message="form.errors?.verification_code"
                />
                <Alert
                    v-if="!form.errors.verification_code"
                    @onClose="alert.verification_code.message = null"
                    :type="alert.verification_code.type"
                ...
                <Alert
                    ...
                    "
                    :message="
                        form.errors?.password || form.errors?.confirm_password
                    "
                />

                <div class="flex justify-end">
                    <Link
                        :href="route('users.login-page')"
                ...
                <Button
                    type="submit"
                    class="w-full py-1 lg:py-1.5 text-center rounded uppercase"
                    >CONTINUE</Button
                >
            </form>
        </Card>
